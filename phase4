...
class Chromosome:
    def __init__(self, _size):  
        ...
        for _ in range(self.chromosome_size):
            self.genes.append(random.randint(0, 9)) #0-9 this time as not binary
    ...
    def mutate(self, gene_index):
             self.genes[gene_index] = random.randint(0, 9) # 0-9 so no long 'flip' index
...
target = [6, 7, 7, 3, 5, 6, 3, 1, 3, 5, 1, 7, 3, 0,
          8, 8, 1, 2, 9, 3, 1, 8, 4, 8, 1, 2, 1, 9, 1, 0] # random generated target string
